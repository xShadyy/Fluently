...
describe("UserHeader", () => {
  ...
  it("renders the UserCard when a user is fetched", async () => {
    ...
    renderWithProviders(<UserHeader />);
    await waitFor(() => {
      expect(screen.getByTestId("user-card")).toHaveTextContent("User: Test User");
    });
  });

  it("renders 'Not logged in' when fetch fails", async () => {
    (global.fetch as unknown as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("Fetch error"));
    renderWithProviders(<UserHeader />);
    await waitFor(() => {
      expect(screen.getByText("Not logged in")).toBeInTheDocument();
    });
